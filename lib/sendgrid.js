...
Mailer.send = function (options, cb) { // eslint-disable-line
    ...
    if (connector.sendgrid) {
        ...
        if (R.is(Array, sendgridEmail.personalizations) && sendgridEmail.personalizations.length > 0) {
            ...
            //personalizations
            if (options.sendGridConfig.personalizations[0].substitutions) {
                var substitutions = options.sendGridConfig.personalizations[0].substitutions
                Object.keys(substitutions).forEach(function (key) {
                    var substitution = new helper.Substitution(key, substitutions[key])
                    sendgridEmail.personalizations[0].addSubstitution(substitution)
                })
            }
            console.log('personalizations ', sendgridEmail.personalizations);
        }
        ...
        if (options.sendGridConfig) {
            ...
            //substitutions
            if (R.is(Array, options.sendGridConfig.substitutions)) {
                R.forEach(function eachSubstitution(arg){
                    console.log('arg ', arg);
                    sendgridEmail.addSubstitution(arg);
                }, options.sendGridConfig.substitutions);
            }
        ...
        console.log('sendgridEmail ', sendgridEmail.toJSON());
